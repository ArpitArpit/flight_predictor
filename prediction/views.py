...
class PredictView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        input_data = {k: v for k, v in serializer.validated_data.items() if k != 'flight_number'}
        flight_number = serializer.validated_data.get('flight_number') or 'N/A'

        predictor = get_predictor()
        
        # 2. Get Base Probability from ML Model (Module 3)
        probability = predictor.predict(input_data)
        if isinstance(probability, dict) and 'error' in probability:
            return Response(probability, status=status.HTTP_503_SERVICE_UNAVAILABLE)
